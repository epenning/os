...
void Process::sleepUntil(uint32_t time, bool isJiffies) {
    Process::disable();

    uint32_t target = time * (isJiffies ? 1 : Pit::hz);
...
void Process::sleepFor(uint32_t seconds) {
    sleepUntil(Pit::seconds() + seconds, false);
}

void Process::sleepForJiffies(uint32_t jiffies) {
    sleepUntil(Pit::jiffies + jiffies, true);
}
